...
let createAndSavePerson = (done) => {
  let person = new Person({name: "Dean Winchester", age: 40, favoriteFoods: ["Pie"]});
...
let arrayOfPeople = [{name: "Dean Winchester", age: 40, favoriteFoods: ["Pie"]}, {name: "Dean Winchester", age: 40, favoriteFoods: ["Pie"]}];

var createManyPeople = (arrayOfPeople, done) => {
  //arrayOfPeople.push({name: "Dean Winchester", age: 40, favoriteFoods: ["Pie"]}); why?
...
var findPeopleByName = function(personName, done) {
  Person.find({name: personName}, (err, arrayOfPersons) => {
    if (err) return console.log(err);
    done(null, arrayOfPersons);
  });
};
...
const removeById = (personId, done) => {
  Person.findOneAndRemove({_id: personId}, (err, personUpdated) => {
    if (err) 
      return console.log(err);
    done(null, personUpdated);
  });  
...
const queryChain = (done) => {
  ...
  Person.find({favoriteFoods: foodToSearch}).sort('name').limit(2).select('-age').exec((err, outcome) => {
    if (err) return console.log(err);
    done(null, outcome);
  });
};
